"""Unit tests for screener data_source routing and hybrid enrichment."""
...
from bist_quant.screening import screener
...
def test_run_response_hybrid_merges_isyatirim_and_enrichment(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(screener, "_resolve_paths", lambda runtime_paths: runtime_paths)
    monkeypatch.setattr(screener, "_load_local_screen_frame", lambda runtime_paths, force_refresh=False: _local_frame())
    ...
    response = screener._run_response(
        {
            "data_source": "hybrid",
            "sort_by": "symbol",
            "sort_desc": False,
...
def test_run_response_uses_isyatirim_source_without_local_loader(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(screener, "_load_local_screen_frame", _fail_local)
    monkeypatch.setattr(
        screener,
        "_load_isyatirim_screen_frame",
...
def test_run_response_rejects_unknown_data_source() -> None:
    with pytest.raises(ValueError):
        screener._run_response({"data_source": "unknown-source"})
