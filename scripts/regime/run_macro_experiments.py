...
import pandas as pd
from macro_features import MacroConfig
from regime_macro_experiments import MacroAugmentedRegime, print_comparison
...
def run_ablation(data_dir: Path, period_name: str = "Full",
    ...
    """
    logger.info(f"\n{'#'*70}")
    logger.info(f"  ABLATION: {period_name}")
    if start_date or end_date:
        logger.info(f"  Period: {start_date or 'start'} → {end_date or 'end'}")
    logger.info(f"{'#'*70}")
    ...
    logger.info(f"Price data: {prices.index[0].date()} → {prices.index[-1].date()} ({len(prices)} days)")
...
def run_walk_forward(data_dir: Path, split_date: str = '2020-01-01') -> dict:
    ...
    """
    logger.info(f"\n{'='*70}")
    logger.info(f"WALK-FORWARD VALIDATION (split at {split_date})")
    logger.info(f"{'='*70}")
    ...
    for param in base_configs:
        ...
        for _, row in param_rows.iterrows():
            if row['Multiplier'] in ('80%', '120%'):
                ...
                logger.info(f"  {param}: {row['Multiplier']} → Sharpe {row['Sharpe']:.2f} "
...
def baseline_parity_test(data_dir: Path) -> bool:
    ...
    """
    logger.info("\n" + "=" * 70)
    logger.info("BASELINE PARITY TEST")
    logger.info("=" * 70)
    ...
    logger.error(f"  Regimes identical:     {'✅ PASS' if regimes_match else '❌ FAIL'}")
    logger.error(f"  Sharpe identical:      {'✅ PASS' if sharpe_match else '❌ FAIL'} "
          f"({simple_results['sharpe_ratio']:.6f} vs {aug_results['sharpe']:.6f})")
    logger.error(f"  CAGR identical:        {'✅ PASS' if cagr_match else '❌ FAIL'} "
...
def print_full_results(results: list):
    ...
    if len(results) > 1:
        logger.info("\nDelta vs Baseline:")
        logger.info(f"{'Mode':<20} {'CAGR Δ':>10} {'Sharpe Δ':>10} {'MaxDD Δ':>10} {'Overrides':>10}")
        logger.info("-" * 65)
        for r in results[1:]:
            logger.info(f"{r['mode']:<20} "
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info("MACRO-AUGMENTED REGIME FILTER EXPERIMENTS")
    ...
    if args.sensitivity:
        ...
        logger.info("\nSENSITIVITY TABLE:")
        logger.info(sens.to_string(index=False))
