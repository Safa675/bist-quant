...
from macro_features import MacroConfig, MacroFeatures
...
def print_comparison(results_list: list):
    ...
    logger.info("\n" + "=" * 90)
    logger.info("MACRO EXPERIMENT COMPARISON")
    logger.info("=" * 90)
...
if __name__ == "__main__":
    ...
    logger.info(f"\nMode: {mode}")
    logger.info(f"CAGR:  {results['cagr']*100:.1f}%")
    logger.info(f"Sharpe: {results['sharpe']:.2f}")
    logger.info(f"MaxDD:  {results['max_dd']*100:.1f}%")
