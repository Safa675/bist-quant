"""Expression-based technical scans over index universes or symbol lists."""
...
LOGGER = logging.getLogger("bist_quant.clients.technical_scan")
...
def resolve_scan_condition(
    *,
    condition: str | None = None,
    template: str | None = None,
) -> str:
    """Resolve a scan expression from a raw condition and/or predefined template name."""
    if template is not None:
        key = str(template).strip().lower()
        if not key:
            raise ValueError("template must be a non-empty string when provided.")
        resolved = PREDEFINED_SCANS.get(key)
        if resolved is None:
    ...
    if not resolved_condition:
        raise ValueError("condition is required when template is not provided.")

    key = resolved_condition.lower()
    if key in PREDEFINED_SCANS:
        return PREDEFINED_SCANS[key]
...
class TechnicalScanner:
    ...
    def scan(
        ...
        *,
        ...
        resolved_condition = resolve_scan_condition(condition=condition, template=template)
