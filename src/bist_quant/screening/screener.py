...
from bist_quant.screening.cache import SCREEN_CACHE, SCREEN_CACHE_TTL_SEC, screen_cache_state_token
...
LOGGER = logging.getLogger("bist_quant.screening.screener")
...
def _load_local_screen_frame(
    ...
    *,
    ...
    now = time.time()
    expected_state_token = screen_cache_state_token(runtime_paths.data_dir)
    cached_frame = SCREEN_CACHE.get("frame")
    cached_as_of = SCREEN_CACHE.get("as_of")
    cached_sector_map = SCREEN_CACHE.get("sector_map")
    cached_close_df = SCREEN_CACHE.get("close_df")
    cached_data_dir = SCREEN_CACHE.get("data_dir")
    cached_state_token = SCREEN_CACHE.get("state_token")
    built_at = float(SCREEN_CACHE.get("built_at", 0.0) or 0.0)
    if (
        not force_refresh
    ...
    SCREEN_CACHE["built_at"] = now
    SCREEN_CACHE["as_of"] = as_of_iso
    SCREEN_CACHE["frame"] = frame.copy()
    SCREEN_CACHE["sector_map"] = dict(sector_map)
    SCREEN_CACHE["close_df"] = close_df.tail(756).copy()
    SCREEN_CACHE["data_dir"] = str(data_dir)
    SCREEN_CACHE["state_token"] = expected_state_token
...
def _run_response(
    ...
    *,
    ...
    close_cache = SCREEN_CACHE.get("close_df")
...
def get_screener_metadata() -> dict[str, Any]:
...
def run_screener(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise ScreeningValidationError("Request payload must be a JSON object.")
    try:
        ...
        raise ScreeningDataError(_friendly_error(exc)) from exc
    except ValueError as exc:
        raise ScreeningValidationError(_friendly_error(exc)) from exc
    except Exception as exc:
        LOGGER.exception("screener.run_failed")
        raise ScreeningExecutionError(
...
def get_stock_filter_metadata() -> dict[str, Any]:
    """Deprecated alias for :func:`get_screener_metadata`."""
    return get_screener_metadata()
...
def run_stock_filter(
    ...
    *,
    ...
    """Deprecated alias for :func:`run_screener`."""
