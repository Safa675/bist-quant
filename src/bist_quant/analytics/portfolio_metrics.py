...
RISK_FREE_RATE = DEFAULT_FALLBACK_RISK_FREE_RATE  # Fallback only
_RISK_FREE_RATE_CACHE: float | None = None


def _coerce_rate_to_decimal(value: float | int | None) -> float | None:
    if value is None:
        return None
    try:
        parsed = float(value)
...
def get_default_risk_free_rate() -> float:
    """Resolve annual risk-free rate as decimal from FixedIncomeProvider."""
    ...
    resolved = None
    try:
        resolved = _coerce_rate_to_decimal(FixedIncomeProvider().get_risk_free_rate())
    except Exception:
        resolved = None

    if resolved is None:
        resolved = float(RISK_FREE_RATE)

    _RISK_FREE_RATE_CACHE = float(resolved)
    return _RISK_FREE_RATE_CACHE


def _resolve_risk_free_rate(risk_free_rate: float | None) -> float:
    if risk_free_rate is None:
        return get_default_risk_free_rate()
    resolved = _coerce_rate_to_decimal(risk_free_rate)
    return float(resolved if resolved is not None else get_default_risk_free_rate())
...
def calculate_sharpe_ratio(
    ...
    resolved_rf = _resolve_risk_free_rate(risk_free_rate)

    # Convert annual risk-free to daily
    daily_rf = (1 + resolved_rf) ** (1 / periods_per_year) - 1
...
def calculate_alpha(
    ...
    resolved_rf = _resolve_risk_free_rate(risk_free_rate)
    expected = resolved_rf + beta * (bench_annual - resolved_rf)
...
def calculate_rolling_metrics(
    ...
    # Rolling Sharpe
    resolved_rf = _resolve_risk_free_rate(risk_free_rate)
    daily_rf = (1 + resolved_rf) ** (1 / TRADING_DAYS_PER_YEAR) - 1
...
class PortfolioAnalytics:
    ...
    def __init__(
        ...
        self.risk_free_rate = _resolve_risk_free_rate(risk_free_rate)
        self.name = name
