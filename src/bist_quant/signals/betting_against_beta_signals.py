...
def build_betting_against_beta_signals(
    ...
    logger.info(f"  Beta calculation window: {beta_window} days (~1 year)")
    logger.info("  Lower beta = Higher score = Defensive stocks")
    ...
    if len(common_dates) < beta_window:
        logger.warning("  ⚠️  Not enough overlap with market returns for beta window")
        return pd.DataFrame(0.0, index=dates, columns=close_df.columns)

    stock_aligned = stock_returns.reindex(common_dates)
    market_aligned = market_returns.reindex(common_dates)

    logger.info("  Calculating rolling betas (vectorized)...")
    ...
    if not beta_panel.empty:
        ...
        if len(latest) > 0:
            ...
            actual_betas = -latest
            logger.info(f"  Latest betas - Mean: {actual_betas.mean():.3f}, Std: {actual_betas.std():.3f}")
            logger.info(f"  Latest betas - Min: {actual_betas.min():.3f}, Max: {actual_betas.max():.3f}")
            
            # Show top 5 low-beta stocks (highest scores)
            top_5 = latest.nlargest(5)
            logger.info(f"  Top 5 low-beta stocks: {', '.join(top_5.index.tolist())}")
            top_5_betas = -top_5
            for ticker, beta in top_5_betas.items():
                logger.info(f"    {ticker}: β = {beta:.3f}")
    
    logger.info(f"  ✅ Betting against beta signals: {beta_panel.shape[0]} days × {beta_panel.shape[1]} tickers")
    return beta_panel
